...
use ark_bn254::Bn254;
use context::{ServiceContext, KEYPAIR_KEY};
...
pub async fn decrypt_ciphertext(
...
) -> Result<Vec<u8>, decrypt::DecryptError> {
    ...
    let ciphertext: Ciphertext<Bn254> = setup::from_bytes(&ciphertext);
    ...
    let keypair =
        context
            .secret_key_store
            .get(KEYPAIR_KEY)
            .ok_or(decrypt::DecryptError::ContextError(
                "Keypair not found".to_string(),
            ))?;

    // Deserialize the secret key
    let secret_key: SecretKey<Bn254> = setup::from_bytes(&keypair.secret_key);
    ...
    let decryption = decrypt::threshold_decrypt_protocol(
        party,
        i as u16,
        threshold as u16,
        num_parties as u16,
